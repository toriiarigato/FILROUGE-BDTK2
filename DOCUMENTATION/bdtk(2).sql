...
--
-- Hôte : 127.0.0.1:3306
-- Généré le : mar. 12 oct. 2021 à 17:17
...
CREATE DEFINER=`root`@`localhost` FUNCTION `login` (`eMail` VARCHAR(30), `mdp` VARCHAR(30)) RETURNS TINYINT(1) BEGIN
DECLARE FLAG BOOLEAN;

    IF (SELECT EMAIL_UTILISATEUR, MOT_DE_PASSE_UTILISATEUR FROM utilisateur
       WHERE eMail = EMAIL_UTILISATEUR 
       AND mdp = MOT_DE_PASSE_UTILISATEUR)
    THEN SET FLAG = TRUE;
    RETURN (SELECT EMAIL_UTILISATEUR, MOT_DE_PASSE_UTILISATEUR FROM utilisateur);
    ELSE SET FLAG = FALSE;
    END IF;
...
(264, 'blabla', 14.5, 79, 53, 4, NULL, NULL, 'fjdklfjld'),
